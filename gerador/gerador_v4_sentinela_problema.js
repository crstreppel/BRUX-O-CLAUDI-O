...
function backupSeExistir(destinoFinal) {
  ...
  if (!fs.lstatSync(destinoFinal).isFile()) return; // evita pasta
...
function processarJson(nomeArquivo) {
  ...
  try {
    ...
    log(`JSON inválido (parse): ${nomeArquivo}`);
    return;
  }
  ...
  for (const item of itens) {
    // 🔒 VALIDAÇÃO FINAL
    if (!item.arquivo || typeof item.arquivo !== "string") {
      log(`JSON inválido: campo 'arquivo' ausente ou inválido`);
      continue;
    }
