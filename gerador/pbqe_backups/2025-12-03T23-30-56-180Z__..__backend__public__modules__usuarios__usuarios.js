...
async function handleCadastro(event) {
  ...
  try {
    ...
    // { sucesso: true, mensagem: 'Usuário criado...', token, codigo }
...
async function handleLogin(event) {
  ...
  try {
    ...
    // Esperado: { sucesso: true, emailVerificado: true/false, ... }
    if (!data.sucesso) {
    ...
    if (!data.emailVerificado) {
      setMessage('msgLogin', 'E-mail ainda não confirmado. Verifique sua caixa de entrada.', 'warning');
      return;
    }
...
async function confirmarEmailPorToken(token) {
  ...
  try {
    const response = await fetch(`${API_BASE}/confirmar-email?token=${encodeURIComponent(token)}`);
    const data = await response.json().catch(() => ({}));

    if (!response.ok || !data.sucesso) {
      setMessage('msgConfirmacao', data.erro || `Erro ao confirmar e-mail (HTTP ${response.status}).`, 'error');
      return;
    }

    setMessage('msgConfirmacao', 'E-mail confirmado com sucesso! Agora você já pode fazer login.', 'success');
  } catch (error) {
...
async function handleReenvio(event) {
  ...
  try {
    const { ok, data, status } = await postJSON(`${API_BASE}/reenviar-email`, {
      email
    });
    ...
    setMessage('msgConfirmacao', data.mensagem || 'E-mail reenviado com sucesso. Verifique sua caixa de entrada.', 'success');
  } catch (error) {
