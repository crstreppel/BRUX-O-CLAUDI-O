...
module.exports = {
  async login(req, res) {
    try {
      ...
      if (!usuario) {
        console.log('[PBQE-LOGIN] Usuario inexistente');
      ...
      // 🔐 PROVA ARGON2ID (sem vazar segredo)
      // Mostra somente o prefixo do hash para confirmar algoritmo ($argon2id$...)
      console.log('[PBQE-LOGIN] Hash prefixo:', (usuario.senha || '').substring(0, 20) + '...');

      const senhaOk = await usuario.validarSenha(senha);

      // Resultado do verify (true/false) — isso é a prova prática
      console.log('[PBQE-LOGIN] Resultado argon2.verify:', senhaOk);
